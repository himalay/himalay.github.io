...
function ReferralLink({ disabled, share, children }: { disabled: boolean; share: string; children: React.ReactNode }) {
  ...
  return (
    <MenuShare href={disabled ? '' : share} onClick={handleClick} disabled={disabled}>
  ...
  useEffect(() => {
    ...
    function handleMenuFloatSettings() {
      ...
      setTimeout(() => {
        ...
        const paragraphOffset = paragraph[0].offsetLeft

        if (!article) return

        // We want to not show the menu float in code blocks
        const codeBlocks = Array.from(article.getElementsByClassName('prism-code'))
        const isHighlightedInCodeBlock = codeBlocks.some((block) => window.getSelection()?.containsNode(block, true))
        ...
        const textPosition = getHighlightedTextPositioning()
        ...
        const offset: { x: number; y: number } = {
          x: height > 29 ? paragraphOffset + paddingOffset : textPosition.x,
          y: textPosition.y - articleBox.y - 160,
        }
    ...
    // attach all events
    events.forEach((event) => window.addEventListener(event, handleMenuFloatSettings))

    return () => {
      // remove all events after mount
      events.forEach((event) => window.removeEventListener(event, handleMenuFloatSettings))
    }
  }, [show])
