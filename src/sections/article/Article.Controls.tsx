...
const DarkModeToggle: React.FC<{}> = () => {
  ...
  return (
    <IconWrapper onClick={toggleColorMode} aria-label="Toggle dark and light mode">
...
const SharePageButton: React.FC<{}> = () => {
  ...
  return (
    <IconWrapper onClick={copyToClipboardOnClick} data-a11y="false" aria-label="Copy URL to clipboard">
      <Icon />
...
const MoonMask = styled.div<{ isDark: boolean }>`
  ...
  transform: translate(${(p) => (p.isDark ? '14px, -14px' : '0, 0')});
  opacity: ${(p) => (p.isDark ? 0 : 1)};
  transition: transform 0.45s ease, ${(p: ThemedProp) => p.theme.colorModeTransition};
`
